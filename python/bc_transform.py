...
class Flows:
    ...
    def list_flows():
        ...
        # if file exists it will be overwritten
        # if file doesn't exist it will be create and written to via stdout
        sys.stdout = open(f"{Flows.path}{Flows.file}", "wt")
    ...
    def clean_data():
        from python import clean_file
        clean_file.clean_file(f'{Flows.path}{Flows.file}',f'{Flows.path}{Flows.file}.csv')

    def create_df():
        df = pd.read_csv(f'{Flows.path}{Flows.file}.csv', on_bad_lines='skip')
        print(df)
        return df
